...
- Feature custom: Transfer fee (cap 100 bp), Custody fee (cap 200 bp), Freeze, Blocklist, Recoverable, Pausable, ContractURIs (websiteURI/reserveInfoURI/contractURI ERC-7572)
- Governance: `AccessControlDefaultAdminRulesUpgradeable` — transfer di `DEFAULT_ADMIN_ROLE` a due fasi con delay (`beginDefaultAdminTransfer` → attesa → `acceptDefaultAdminTransfer`); `grantRole`/`revokeRole` su `DEFAULT_ADMIN_ROLE` revertono sempre
...
forge test                # 327 test Foundry: unit + fuzz + invariant (~18s)
...
3. **Tutti i 546 test devono passare** (219 Hardhat + 327 Foundry) prima di ogni commit. La CI (.github/workflows/test.yml) esegue `forge fmt --check`, `forge build --sizes`, `forge test -vvv`.
