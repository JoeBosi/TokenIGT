...
forge test                # 335 test Foundry: unit + fuzz + invariant (~18s)
...
3. **Tutti i 561 test devono passare** (226 Hardhat + 335 Foundry) prima di ogni commit. La CI (.github/workflows/test.yml) esegue `forge fmt --check`, `forge build --sizes`, `forge test -vvv`.
