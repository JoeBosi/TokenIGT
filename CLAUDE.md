...
forge test                # 258 test Foundry: unit + fuzz + invariant (~18s)
...
3. **Tutti i 447 test devono passare** (189 Hardhat + 258 Foundry) prima di ogni commit. La CI (.github/workflows/test.yml) esegue `forge fmt --check`, `forge build --sizes`, `forge test -vvv`.
